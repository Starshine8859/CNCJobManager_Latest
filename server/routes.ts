...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.use("/api/part-checklists", requireAuth, partChecklistsRouter)
  ...
  app.get("/api/jobs", requireAuth, async (req, res) => {
    try {
      ...
      const jobs = await storage.getAllJobs(search as string, status as string)
  ...
  app.post("/api/materials/:id/add-sheets", requireAuth, async (req, res) => {
    try {
      const materialId = Number.parseInt(req.params.id)
  ...
  app.post("/api/materials/:materialId/sheets/:sheetIndex", requireAuth, async (req, res) => {
    try {
      const materialId = Number.parseInt(req.params.materialId)
      const sheetIndex = Number.parseInt(req.params.sheetIndex)
  ...
  app.delete("/api/recuts/:id", requireAuth, async (req, res) => {
    try {
      const recutId = Number.parseInt(req.params.id)
  ...
  app.delete("/api/cutlists/:id", requireAuth, async (req, res) => {
    try {
      const cutlistId = Number.parseInt(req.params.id)
  ...
  app.post("/api/supplies", requireAuth, async (req, res) => {
    try {
      ...
      res
        .status(400)
        .json({ message: "Invalid supply data", error: error instanceof Error ? error.message : String(error) })
    }
  })
  ...
  app.get("/api/supplies/:id/vendors", requireAuth, async (req, res) => {
    try {
      const supplyId = Number.parseInt(req.params.id)
  ...
  app.put("/api/purchase-orders/:id/status", requireAuth, async (req, res) => {
    try {
      ...
      await db.update(purchaseOrders).set(updateData).where(eq(purchaseOrders.id, orderId))
      ...
      if (status === "received") {
        ...
        for (const item of orderItems) {
          ...
          const existingLocation = await db
            .select()
            .from(supplyLocations)
            .where(and(eq(supplyLocations.supplyId, item.supplyId), eq(supplyLocations.locationId, item.locationId)))
  ...
  app.put("/api/purchase-orders/:id/items/:itemId", requireAuth, async (req, res) => {
    try {
      ...
      const [current] = await db
        ...
        .from(purchaseOrderItems)
        .where(eq(purchaseOrderItems.id, Number.parseInt(itemId)))
      const target = Math.max(0, Math.min(Number.parseInt(receivedQuantity), current?.orderQuantity || 0))
      await db
      ...
      if (!recipientEmail) {
        if (items.length > 0 && items[0].vendorId) {
          const [vendorRow] = await db
            ...
            .from(vendors)
            .where(eq(vendors.id, items[0].vendorId))
  ...
  app.get("/api/users", requireSuperAdmin, async (req, res) => {
    try {
      ...
      res.json(
        users.map((u) => ({ id: u.id, username: u.username, email: u.email, role: u.role, createdAt: u.createdAt })),
      )
    } catch (error) {
  ...
  */
  ...
  app.post("/api/inventory/transfer", requireAuth, async (req, res) => {
    try {
      ...
      const sourceStock = await db
        .select()
        .from(supplyLocations)
        .where(and(eq(supplyLocations.supplyId, supplyId), eq(supplyLocations.locationId, fromLocationId)))
      ...
      const destLocation = await db
        .select()
        .from(supplyLocations)
        .where(and(eq(supplyLocations.supplyId, supplyId), eq(supplyLocations.locationId, toLocationId)))
  ...
  app.get("/api/inventory/movements", requireAuth, async (req, res) => {
    try {
      ...
      const conditions = []
      if (supplyId) {
        conditions.push(eq(inventoryMovements.supplyId, Number.parseInt(supplyId as string)))
      }
      if (locationId) {
        conditions.push(
          or(
            eq(inventoryMovements.fromLocationId, Number.parseInt(locationId as string)),
            eq(inventoryMovements.toLocationId, Number.parseInt(locationId as string)),
          ),
        )
      }
      if (fromDate) {
  ...
  app.get("/api/emails", requireAuth, async (req, res) => {
    try {
      ...
      let results = await db
        .select()
        .from(emails)
        .where(eq(emails.folder, folder as string))
      if (q) {
  ...
  app.post("/api/emails", requireAuth, async (req, res) => {
    try {
      ...
      const defaultFrom =
        ...
        (req.session?.user?.email ? req.session.user.email : "noreply@cnc-job-manager.local")
  ...
  app.get("/api/inventory/need-to-purchase", requireAuth, async (req, res) => {
    try {
      ...
      const onOrderAgg = await db
        ...
        .where(or(eq(purchaseOrders.status, "ordered"), eq(purchaseOrders.status, "partially_received")))
      ...
      const manualItems: Array<{ supplyId: number; locationId: number; quantity: number }> =
        (req.session as any).manualNeedToPurchase || []
      ...
      if (missingKeys.length > 0) {
        const parts = missingKeys.map((k) => k.split("-").map((n) => Number.parseInt(n, 10))).filter(([s, l]) => s && l)
  ...
  app.post("/api/inventory/need-to-purchase/manual", requireAuth, async (req, res) => {
    try {
      ...
      const sid = Number.parseInt(supplyId)
      const lid = Number.parseInt(locationId)
      const qty = Math.max(1, Number.parseInt(quantity))
      if (!sid || !lid || !qty) {
  ...
  app.post("/api/inventory/alerts/:id/resolve", requireAuth, async (req, res) => {
    try {
      ...
      await db.update(inventoryAlerts).set({ isResolved: true }).where(eq(inventoryAlerts.id, alertId))
